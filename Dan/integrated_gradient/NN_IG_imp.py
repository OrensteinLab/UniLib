...
def get_gradients(model, sample_inputs, target_range=None, jacobian=False):
    ...
    Args:
        sample_inputs (ndarray) or list of (ndarray): model sample inputs 
...
def get_integrated_gradients(model, sample_inputs, target_range=None, baselines=None, num_steps=50, multiple_samples=False):
    ...
    Args:
        ...
        sample_inputs (ndarray) or list of (ndarray): Original sample inputs to the model 
        target_range (slice): Target range - grdient of Target range  with respect to the input
        baseline (ndarray) or list of (ndarray): The baseline to start with for interpolation
        num_steps: Number of interpolation steps between the baseline
            and the input used in the computation of integrated gradients. These
            steps along determine the integral approximation error. By default,
            num_steps is set to 50.
		multiple_samples (boolean): compute IG for multiple samples at once
    Returns:
